...
//	private  String filePath;
	...
	// 数据的属性行的名字
	private   String[] attrNames ={"Rid","Age","Income","Distance","Weather","Result"};
	// 所有属性的类型总数,在这里就是data源数据的列数
	private  int attrNum = attrNames.length ;
	...
	/*	UserDaoImpl dao = new UserDaoImpl();
		dlList =dao.getDLList();*/
		dlList =dltableDao.getDLList();
		String[] tempArray = null;
		
		data=new String[dlList.size()+1][];
		data[0] = attrNames;
...
//		
	...
	public  void startBuildingTree() {
		...
		List<DLTable> dlTables = setIndexAndAlpah(rootNode, 0, false);
		List<DLTable> dlTables1 = setIndexAndAlpah(rootNode, 0, false);
		dltableDao.savaDLTable(dlTables1);
		System.out.println("\n剪枝前：");
		showDecisionTree(rootNode, 1);
		System.out.println();
		System.out.println(dlTables1);
		List<DLTable> dlTables2 = setIndexAndAlpah(rootNode, 0, true);
		System.out.println("\n剪枝后：");
		showDecisionTree(rootNode, 1);
		System.out.println();
		System.out.println(dlTables2);
	}
	 ...
	 *            是否需要剪枝
	 */
	private List<DLTable> setIndexAndAlpah(AttrNode node, int index, boolean ifCutNode) {
		List<DLTable> dlTables = new ArrayList<DLTable>();
		AttrNode tempNode;
		DLTable dlTable;
		// 最小误差代价节点，即将被剪枝的节点
		AttrNode minAlphaNode = null;
		DLTable minDLTable = null;
		...
		while (nodeQueue.size() > 0) {
			...
			while (cur < last) {
				...
				dlTable = loadTable(tempNode);
			
				dlTable.setLevelNum(level);
				dlTables.add(dlTable);
				
				if (tempNode.getChildAttrNode() != null) {
					for (AttrNode childNode : tempNode.getChildAttrNode()) {
						nodeQueue.add(childNode);	
						// 设置父节点index
						childNode.setParentNodeIndex(tempNode.getNodeIndex());						
					}
		...
		if (ifCutNode) {
			...
			//保存根节点	
			if(minDLTable.getRchildIndex()!=0){
				CutDLTable(DLTable.getByIndex(minDLTable.getLchildIndex()),dlTables);
			}	
			if(minDLTable.getRchildIndex()!=0){
				CutDLTable(DLTable.getByIndex(minDLTable.getRchildIndex()),dlTables);
			}
			/*
			 * 不保存根节点
			 * 
			 * CutDLTable(minDLTable, dlTables);
			}*/		
		}
	...
	//递归剪枝
	private void CutDLTable(DLTable minDLTable, List<DLTable> dlTables) {
		if(minDLTable.getNodeIndex()==0)
			return;
		int lChildIndex = minDLTable.getLchildIndex();
		int rChildIndex = minDLTable.getRchildIndex();
		if(lChildIndex!=0){
			CutDLTable(DLTable.getByIndex(lChildIndex),dlTables);
		}
		if(rChildIndex!=0){
			CutDLTable(DLTable.getByIndex(rChildIndex),dlTables);
		}
		dlTables.remove(minDLTable);
	}

	
	//加载DLTable
	private DLTable loadTable(AttrNode node) {
		int nodeIndex = node.getNodeIndex();	
		DLTable dlTable = new DLTable(nodeIndex);
		dlTable.setLayerType(node.getParentAttrValue());
		dlTable.setTid(nodeIndex);
		dlTable.setValueType(node.getAttrName());
		dlTable.setParentNodeIndex(node.getParentNodeIndex());
		
		if (node.getChildAttrNode() != null) {
			if (node.getChildAttrNode()[0] != null)
				dlTable.setLchildIndex(node.getChildAttrNode()[0]
						.getNodeIndex());
			if (node.getChildAttrNode()[1] != null)
				dlTable.setRchildIndex(node.getChildAttrNode()[1]
						.getNodeIndex());
		} else {
			dlTable.setResult(data[Integer.parseInt(node.getDataIndex().get(0))][attrNames.length-1]);
			System.out.println(node.getDataIndex());
			dlTable.setDataIndexArray(node.getDataIndex());
			System.out.println(node.getDataIndex());		
			String indexArrays = "";
				if (node.getDataIndex() != null) {
					for (String dataIndex : node.getDataIndex()) {
						indexArrays += dataIndex.toString() + ",";
					}
				}					
			dlTable.setDataIndexArrays(indexArrays);
		}
		return dlTable;
	}
